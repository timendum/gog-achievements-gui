							...
							<img
								src={
									achievement.date_unlocked
										? achievement.image_url_unlocked
										: achievement.image_url_locked
								}
							...
							<div className="flex-1 text-left">
								...
								{achievement.date_unlocked && (
									<p className="text-xs text-green-600 mt-1">
										Unlocked: {formatUnlockedDate(achievement.date_unlocked)}
									</p>
								)}
							</div>
						</button>
							...
							{pendingChanges.locked.length > 0 && (
								<div className="mb-4">
									<h3 className="font-semibold text-green-700 mb-2">
										Achievements to Unlock ({pendingChanges.locked.length}):
									</h3>
									<div className="space-y-2">
										{pendingChanges.locked.map((id) => {
											...
											return ach ? (
												<div
													key={id}
													className="flex gap-3 items-center bg-green-50 p-3 rounded"
												>
													<img
														src={ach.image_url_unlocked}
														alt={ach.name}
							...
							{pendingChanges.unlocked.length > 0 && (
								<div>
									<h3 className="font-semibold text-red-700 mb-2">
										Achievements to Lock ({pendingChanges.unlocked.length}):
									</h3>
									<div className="space-y-2">
										{pendingChanges.unlocked.map((id) => {
											...
											return ach ? (
												<div
													key={id}
													className="flex gap-3 items-center bg-red-50 p-3 rounded"
												>
													<img
														src={ach.image_url_locked}
														alt={ach.name}
														className="w-12 h-12 rounded"
													/>
													<div>
